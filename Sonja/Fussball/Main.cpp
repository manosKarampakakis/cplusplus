...
int main() {
	...
	lehmann->setAbwehrKommunikation(10);
	lehmann->setStrafRaumBeherrschung(1);
	lehmann->setLinienQualitaet(3);

	boka->setBallBeherrschung(3);
	boka->setSchussGenauigkeit(5);
	boka->setSchussKraft(2);
	boka->setVerteidigung(6);
	boka->setStarkerFuss(rechts);

	veh->setLizenzStufe(6);
